...
namespace Autowont.PageLocator
{
    ...
    {
        protected virtual IAWPage CreatePage(Type pageType)
        {
            return Activator.CreateInstance(pageType) as IAWPage;
        }
        ...
        public Page ResolvePage(IViewModel viewModel)
        {
            ...
            if (!(page is IAWPage))
                throw new ArgumentException("Page for '" + viewModel.GetType().Name +
                                            "' should be of type 'AWPage' instead of '" +
                                            pageType.Name + "'");
        ...
        {
            var tabbedPage = new AWTabbedPage { BindingContext = tabbedViewModel };
            ...
            foreach (var type in types)
            {
                ...
                var childViewModel = childPage.BindingContext as ViewModel;
            ...
            tabbedPage.CurrentPageChanged += (sender, eventArgs) =>
            {
                ...
                tabbedViewModel.OnSelectedTabChanged((ViewModel)tabbedPage.CurrentPage.BindingContext);
            };
